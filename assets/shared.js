...
window.openProjectModal = function (data) {
  ...
  const imgStart = first.img || '';

  const dotsHTML = slides.length > 1
    ? slides.map((_, i) => `<button class="modal-thumb-dot${i===0?' active':''}" data-idx="${i}" aria-label="Slide ${i+1}"></button>`).join('')
    : '';
  ...
  overlay.innerHTML = `
    <div class="modal-box" role="dialog" aria-modal="true">
      ...
      <div class="modal-gallery">
        <div class="modal-gallery-main" style="position:relative;overflow:hidden;">
          <div class="modal-gallery-bg" id="mgBg" style="position:absolute;inset:0;background:${bgStart};transition:background 0.4s;"></div>
          ${imgStart ? `<img id="mgImg" src="${imgStart}" alt="" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0.85;transition:opacity 0.4s;" onerror="this.style.display='none'">` : '<div id="mgImg"></div>'}
          <span class="modal-gallery-label" id="mgLabel" style="position:relative;z-index:2;">${data.label||''}</span>
        </div>
        ${dotsHTML ? `<div class="modal-thumbs">${dotsHTML}</div>` : ''}
      </div>
  ...
  // Gallery dot navigation with image swap
  if (slides.length > 1) {
    overlay.querySelectorAll('.modal-thumb-dot').forEach(dot => {
      dot.addEventListener('click', () => {
        const s = slides[+dot.dataset.idx];
        document.getElementById('mgBg').style.background = s.gradient || bgStart;
        document.getElementById('mgLabel').textContent = s.label || data.label;
        const imgEl = document.getElementById('mgImg');
        if (imgEl && imgEl.tagName === 'IMG') {
          imgEl.style.opacity = '0';
          setTimeout(() => {
            imgEl.src = s.img || '';
            imgEl.style.opacity = s.img ? '0.85' : '0';
          }, 200);
        }
        overlay.querySelectorAll('.modal-thumb-dot').forEach(d => d.classList.remove('active'));
